...
class Cryptocurrency {
	constructor(symbol, image, total_volume) {
		this.symbol = symbol;
		this.image = image;
	...
	static from_json(json) {
		return new Cryptocurrency(json.symbol, json.image, json.total_volume);
	}

	render() {
		let price = NUMBER_FORMATTER.format(this.total_volume);
		let date_string = DATE_FORMATTER.format(this.date);
		const html = `
			<section class="chart">
				<header class="chart__header">
					<img class="chart__header__profile" alt="${this.symbol}" src="${this.image}" />
					<div class="chart__header__details">
						<h2 class="chart__header__details__subtitle" lang="en">${this.symbol}</h2>
